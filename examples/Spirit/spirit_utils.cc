...
void ikSpiritStand(drake::multibody::MultibodyPlant<double>& plant,
  ...
  if(sag_spine){
    ik.get_mutable_prog()->AddLinearConstraint(
        (ik.q())(positions_map.at("sag_spine")) == 0);
  }
  if(cor_spine){
    ik.get_mutable_prog()->AddLinearConstraint(
        (ik.q())(positions_map.at("cor_spine")) == 0);
  }
...
void setSpiritJointLimits(drake::multibody::MultibodyPlant<T> & plant, 
  ...
  if(sag_spine){
    ...
    for(int i = 0; i<N_knotpoints;i++){
      auto xi = trajopt.state(i);
      trajopt.AddBoundingBoxConstraint(-M_PI / 3.0,M_PI/3.0,xi(positions_map.at("sag_spine")));
    }
  }
  if(cor_spine){
    auto positions_map = multibody::makeNameToPositionsMap(plant);
    ...
    for(int i = 0; i<N_knotpoints;i++){
      auto xi = trajopt.state(i);
      trajopt.AddBoundingBoxConstraint(-M_PI / 3.0,M_PI/3.0,xi(positions_map.at("cor_spine")));
    }
  }
}
//   \SETSPIRITJOINTLIMITS 
  ...
  for(int iKnot = 0; iKnot<N_knotpoints;iKnot++){
    ...
    if(sag_spine){
      trajopt.AddBoundingBoxConstraint(
          ...
          actuatorLimit * gear_spine / 6,
          ui(actuators_map.at("motor_sag_spine")));
    }
    if(cor_spine){
      trajopt.AddBoundingBoxConstraint(
          -actuatorLimit * gear_spine / 6,
          actuatorLimit * gear_spine / 6,
          ui(actuators_map.at("motor_cor_spine")));
    }
  ...
  for(const auto& x_traj : x_trajs) {
    ...
    for (int knot_index = 0; knot_index < knot_points.size() - 1; knot_index++) {
      ...
      if(sag_spine){
        // Spine work
        Q = Q_spine;
        double actuation_low = u_low(actuator_map.at("motor_sag_spine"));
        double actuation_up = u_up(actuator_map.at("motor_sag_spine"));

        double velocity_low = x_low(n_q + velocities_map.at("sag_spinedot"));
        double velocity_up = x_up(n_q + velocities_map.at("sag_spinedot"));
      ...
      if(cor_spine){
        // Spine work
        Q = Q_spine;
        double actuation_low = u_low(actuator_map.at("motor_cor_spine"));
        double actuation_up = u_up(actuator_map.at("motor_cor_spine"));

        double velocity_low = x_low(n_q + velocities_map.at("cor_spinedot"));
        double velocity_up = x_up(n_q + velocities_map.at("cor_spinedot"));
  ...
  for (int mode_index = 0; mode_index < trajopt.num_modes(); mode_index++) {
    for (int knot_index = 0; knot_index < trajopt.mode_length(mode_index)-1; knot_index++) {
      ...
      if(sag_spine){
        ...
        int act_int = actuator_map.at("motor_sag_spine");
        int vel_int =  n_q + velocities_map.at("sag_spinedot");
      ...
      if(cor_spine){
        auto joint_work_cost = std::make_shared<JointWorkCost>(plant,  Q, cost_work_gain,4);
        int act_int = actuator_map.at("motor_cor_spine");
        int vel_int =  n_q + velocities_map.at("cor_spinedot");
