...
template <class Y> 
SpiritBoundS2s<Y>::SpiritBoundS2s(){
}
...
template <class Y>
void SpiritBoundS2s<Y>::config(std::string yaml_path, std::string saved_directory, int index,MultibodyPlant<Y>* plant){
...
/// \param plant: robot model
template <class Y>
void SpiritBoundS2s<Y>::generateInitialGuess(MultibodyPlant<Y>& plant){
...
template <class Y>
void SpiritBoundS2s<Y>::addCostLegs(MultibodyPlant<Y>& plant,
...
template <class Y>
std::vector<drake::solvers::Binding<drake::solvers::Cost>> SpiritBoundS2s<Y>::addCost(
...
/// \param trajopt trajectory optimization problem to be solved
template <class Y>
void SpiritBoundS2s<Y>::addConstraints(
                    MultibodyPlant<Y>& plant,
                    dairlib::systems::trajectory_optimization::Dircon<Y>& trajopt){
...
template <class Y>
void SpiritBoundS2s<Y>::setUpModeSequence(){
  ...
  this->addModeToSequenceVector("flight",Eigen::Vector3d::UnitZ(),Eigen::Vector3d::Zero(),0.02, 1);
  this->addModeToSequenceVector("front_stance",finalStand.normal(),   finalStand.offset(),0.02, 0.1);
  this->addModeToSequenceVector("stance",finalStand.normal(),   finalStand.offset(),0.02, 0.1);
}
...
template <class Y>
void SpiritBoundS2s<Y>::run(MultibodyPlant<Y>& plant,
...
template class SpiritBoundS2s<double>;
}  // namespace dairlib
